...
public class Row {
    ...
    public String getRowInfo(int row, int col, boolean isThisLine){
        String rowString="";
        for(int i = 0; i < nodes.length; i++){
            if(isHere(isThisLine,i,col)){
                rowString += nodes[i].getNodeInfo()+"* ";
                continue;
            }
            rowString += nodes[i].getNodeInfo()+"  ";
        }
    ...
    private boolean isHere(boolean isThisLine, int curPrintCol, int col){
        return isThisLine && curPrintCol == col;
    }
}